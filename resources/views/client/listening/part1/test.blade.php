				...
				<form   action="#" id="fulltest_part_head" method="post" >
					...
					<div class="question fullest_page_{{ $dem+1 }} part1_{{ $dem+1 }}" data-page="1"  id="test_question_{{ $part1s->get(0)->id }}" data-part="part1">
						<div class="text-center" >
							...
							<div class="col-md-12">
								<audio controls>
									<source src="medias/{{ $part1s->get(0)->media->mediaFile }}" type="audio/mpeg">
									</audio>
								</div>
							</div>
							<div class="text-center script_answer_{{ $dem+1 }}" style="backgroud:red; display: none">
								<p>{{ $part1s->get(0)->media->script_answer }}/p>   
								</div>
								<p>Question: {{ $dem+1 }}</p>
								<ol id="que" type="A"  >
									<li>
										<input type="radio" data-cau="{{ $dem+1 }}" data-part="1" data-question="{{ $part1s->get(0)->id }}" name="answer[{{ $dem }}]" value="{{ $part1s->get(0)->id }}" class="sg-replace-icons radio_answer_a{{ $dem+1 }}">
										<label for=""></label>
									</li>
									<li>
										<input type="radio" data-cau="{{ $dem+1 }}" data-part="1" data-question="{{ $part1s->get(0)->id }}" name="answer[{{ $dem }}]" value="{{ $part1s->get(0)->id }}" class="sg-replace-icons radio_answer_b{{ $dem+1 }}"> 
										<label>
										</label>
									</li>
									<li>
										<input type="radio" data-cau="{{ $dem+1 }}" data-part="1" data-question="{{ $part1s->get(0)->id }}" name="answer[{{ $dem }}]" value="{{ $part1s->get(0)->id }}" class="sg-replace-icons radio_answer_c{{ $dem+1 }}"> 
										<label>
										</label>
									</li>
									<li>
										<input type="radio" data-cau="{{ $dem+1 }}" data-part="1" data-question="{{ $part1s->get(0)->id }}" name="answer[{{ $dem }}]" value="{{ $part1s->get(0)->id }}" class="sg-replace-icons radio_answer_d{{ $dem+1 }}"> 
										<label>
										</label>
									</li>
								</ol>
							...
							<?php
							for ($idx = 1; $idx < count($part1s); $idx++) {
